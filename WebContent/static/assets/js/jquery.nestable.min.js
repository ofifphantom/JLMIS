/*
 * ! Nestable jQuery Plugin - Copyright (c) 2012 David Bushell -
 * http://dbushell.com/ Dual-licensed under the BSD or MIT licenses
 */
(function(e, g, h, c) {
	...
	var b = (function() {
		...
		n.appendChild(m);
		var l = g.getComputedStyle
				&& g.getComputedStyle(m, "").pointerEvents === "auto";
		n.removeChild(m);
		return !!l
	})();
	var k = f ? "touchstart" : "mousedown", j = f ? "touchmove" : "mousemove", a = f
			? "touchend"
			: "mouseup";
	...
	i.prototype = {
		init : function() {
			...
			o.el.on("click", "button", function(s) {
						...
						var r = e(s.currentTarget), q = r.data("action"), p = r
			...
			var l = function(q) {
				...
				if (!p.length || o.dragEl || (!f && q.button !== 0)
						|| (f && q.touches.length !== 1)) {
					return
				}
				q.preventDefault();
		...
		serialize : function() {
			...
			step = function(r, p) {
				...
				o.each(function() {
							var s = e(this), u = e.extend({}, s.data()), t = s
									.children(l.options.listNodeName);
							if (t.length) {
		...
		dragStart : function(q) {
			var m = this.mouse, p = e(q.target).closest("."
					+ this.options.handleClass), o = p
			...
			this.dragEl = e(h.createElement(this.options.listNodeName))
		...
		dragMove : function(s) {
			...
			if (u.dirAx && u.distAxX >= m.threshold) {
				...
				if (u.distX < 0) {
					...
					if (!q.length) {
						...
						this.placeEl.closest(m.itemNodeName)
								.after(this.placeEl);
			...
			this.pointEl = e(h.elementFromPoint(s.pageX - h.body.scrollLeft,
					s.pageY - (g.pageYOffset || h.documentElement.scrollTop)));
			if (!b) {
			...
			var r = this.pointEl.closest("." + m.rootClass), w = this.dragRootEl
					.data("nestable-id") !== r.data("nestable-id");
			if (!u.dirAx || w || o) {
				if (w && m.group !== r.data("nestable-group")) {
					return
				}
				p = this.dragDepth - 1
						+ this.pointEl.parents(m.listNodeName).length;
				if (p > m.maxDepth) {
					return
				}
				var v = s.pageY < (this.pointEl.offset().top + this.pointEl
						.height()
						/ 2);
				x = this.placeEl.parent();
				if (o) {
					t = e(h.createElement(m.listNodeName))
							.addClass(m.listClass);
					t.append(this.placeEl);
