(function($) {
	$.fn.inputlimiter = function(options) {
		...
		var inputlimiterKeyup = function(e) {
			...
			var charsRemaining = (opts.limit - count > 0
					? opts.limit - count
					: 0), remText = opts.remTextFilter(opts, charsRemaining), limitText = opts
					.limitTextFilter(opts);
			...
			if (opts.boxAttach) {
				...
				limitText = limitText.replace(/\%s/g, (opts.limit === 1
								? ''
								: 's'));
			...
			if (opts.limitBy.toLowerCase() === "words") {
				return (value.length > 0 ? $.trim(value).replace(/\ +(?= )/g,
						'').split(' ').length : 0);
			}
		...
		$(this).each(function(i) {
			var $this = $(this);
			if ((!options || !options.limit) && opts.useMaxlength
					&& parseInt($this.attr('maxlength')) > 0
					&& parseInt($this.attr('maxlength')) != opts.limit) {
				$this.inputlimiter($.extend({}, opts, {
							limit : parseInt($this.attr('maxlength'))
						}));
			} else {
				if (!opts.allowExceed && opts.useMaxlength
						&& opts.limitBy.toLowerCase() === "characters") {
	...
	$.fn.inputlimiter.remtextfilter = function(opts, charsRemaining) {
		...
		remText = remText.replace(/\%s/g, (opts.zeroPlural
						? (charsRemaining === 1 ? '' : 's')
						: (charsRemaining <= 1 ? '' : 's')));
		return remText;
	};
