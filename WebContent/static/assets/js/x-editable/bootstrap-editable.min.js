/*
 * ! X-editable - v1.4.6 In-place editing with Twitter Bootstrap, jQuery UI or
 * pure jQuery http://github.com/vitalets/x-editable Copyright (c) 2013 Vitaliy
 * Potapov; Licensed MIT
 */
(function(b) {
	var a = function(d, c) {
	...
	a.prototype = {
		...
		render : function() {
			...
			b.when(this.input.render()).then(b.proxy(function() {
				...
				this.$form.find(".editable-cancel").click(b.proxy(this.cancel,
						this));
				if (this.input.error) {
					...
					var c = (this.value === null || this.value === undefined || this.value === "")
		...
		submit : function(g) {
			...
			b.when(this.save(d)).done(b.proxy(function(e) {
				this.isSaving = false;
				var h = typeof this.options.success === "function"
						? this.options.success.call(this.options.scope, e, f)
						: null;
				if (h === false) {
					this.error(false);
					this.showForm(false);
				...
				this.$div.triggerHandler("save", {
						...
						if (typeof this.options.error === "function") {
							e = this.options.error.call(this.options.scope, h,
									f)
						} else {
							e = typeof h === "string" ? h : h.responseText
									|| h.statusText || "Unknown error!"
						}
						this.error(e);
						this.showForm()
					}, this))
	...
	b.prototype = {
		...
		init : function(e, d) {
			...
			if (!c(document).data("editable-handlers-attached")) {
				...
				c(document).on("click.editable", function(j) {
					var f = c(j.target), g, h = [".editable-container",
							".ui-datepicker-header", ".datepicker",
							".modal-backdrop",
		...
		splitOptions : function() {
			...
			if (!c.fn[this.containerName]) {
				throw new Error(this.containerName
			...
			this.$form.editableform(this.formOptions).on({
				...
				rendered : c.proxy(function() {
							this.$element.triggerHandler("shown",
									c(this.options.scope).data("editable"))
						}, this)
			}).editableform("render")
		},
		show : function(d) {
			...
			c(".editable-open").each(function(g, h) {
						...
						if (e.options.onblur === "cancel") {
							...
							if (e.options.onblur === "submit") {
								f.data("editableContainer").tip().find("form")
										.submit()
							}
						}
					})
...
(function(b) {
		...
		init : function() {
			...
			switch (this.options.autotext) {
				case "always" :
					...
					break;
				case "auto" :
					d = !b.trim(this.$element.text()).length
	...
	b.fn.editable = function(h) {
				...
				if (arguments.length === 2 && arguments[1] === true) {
					...
					this.each(function() {
								...
								if (k && k.value !== undefined
										&& k.value !== null) {
									c[k.options.name] = k.input
											.value2submit(k.value)
								}
							})
				}
				return c;
			case "submit" :
				var f = arguments[1] || {}, j = this, i = this
						.editable("validate"), d;
		...
		return this.each(function() {
					var m = b(this), l = m.data(e), k = typeof h === "object"
							&& h;
					if (k && k.selector) {
						l = new a(this, k);
						return
...
(function(b) {
	...
	b.extend(a.prototype, {
				...
				doPrepend : function() {
					...
					if (b.isArray(this.prependData)
							&& b.isArray(this.sourceData)) {
						this.sourceData = this.prependData
								.concat(this.sourceData)
					}
				},
				renderList : function() {
				},
	...
	b.extend(a.prototype, {
				...
				renderClear : function() {
					if (this.options.clear) {
						...
						this.$input.after(this.$clear).css("padding-right", 24)
								.keyup(b.proxy(function(d) {
											if (~b.inArray(d.keyCode, [40, 38,
															9, 13, 27])) {
												return
											}
											clearTimeout(this.t);
				...
				toggleClear : function(d) {
					...
					var c = this.$input.val().length, f = this.$clear
							.is(":visible");
					if (c && !f) {
						this.$clear.show()
					}
...
(function(b) {
	...
	b.extend(a.prototype, {
		...
		value2str : function(c) {
			return b.isArray(c)
					? c.sort().join(b.trim(this.options.separator))
					: ""
		},
	...
	a.extend(b.prototype, {
		...
		value2html : function(f, c) {
			...
			if (this.options.select2.tags) {
				...
				if (this.sourceData) {
					e = a.fn.editableutils.itemsByValue(f, this.sourceData,
							this.idFunc)
				} else {
				}
			}
...
(function(b) {
	...
	a.prototype = {
		...
		init : function() {
			...
			this.$widget = b('<span class="combodate"></span>').html(this
					.getTemplate());
			this.initCombos();
		...
		getTemplate : function() {
			...
			b.each(this.map, function(e, d) {
						d = d[0];
						var g = new RegExp(d + "+"), f = d.length > 1 ? d
								.substring(1, 2) : d;
						c = c.replace(g, "{" + f + "}")
					});
			c = c.replace(/ /g, "&nbsp;");
		...
		fillHour : function() {
			var e = this.initItems("h"), f, h, k = this.options.template
					...
					.toLowerCase().indexOf("hh") !== -1, g = k ? 1 : 0, c = k
					? 12
					: 23;
			for (h = g; h <= c; h++) {
		...
		setValue : function(f) {
			...
			function g(h, i) {
				...
				h.children("option").each(function(l, k) {
							...
							n = Math.abs(m - i);
							if (typeof j.distance === "undefined"
									|| n < j.distance) {
								j = {
									value : m,
									distance : n
		...
		highlight : function(c) {
			if (!c.isValid()) {
				if (this.options.errorClass) {
					...
					if (!this.borderColor) {
						this.borderColor = this.$widget.find("select")
								.css("border-color")
					}
...
(function(a) {
	...
	a.extend(b.prototype, {
				...
				activate : function() {
					this.$input.siblings(".combodate").find("select").eq(0)
							.focus()
				},
				autosubmit : function() {
				}
...
(function(a) {
	a.extend(a.fn.editableContainer.Popup.prototype, {
		...
		innerCss : a.fn.popover
				&& a(a.fn.popover.Constructor.DEFAULTS.template).find("p").length
				? ".popover-content p"
				: ".popover-content",
		initContainer : function() {
	...
	(function() {
				...
				f = typeof this.options.placement === "function"
						? this.options.placement.call(this, g[0],
								this.$element[0])
				...
				switch (f) {
					case "bottom" :
						if ((l.top + h) > (a(window).scrollTop() + a(window)
								.height())) {
							if (c.top > a(window).scrollTop()) {
								f = "top"
							} else {
								if ((d.left + b) < (a(window).scrollLeft() + a(window)
										.width())) {
									f = "right"
								} else {
...
(function(g) {
	...
	h.prototype = {
		...
		_buildEvents : function() {
			...
			this._secondaryEvents = [[this.picker, {
						...
						mousedown : g.proxy(function(n) {
									if (!(this.element.is(n.target)
											|| this.element.find(n.target)
													.size()
											|| this.picker.is(n.target) || this.picker
											.find(n.target).size())) {
										this.hide()
									}
								}, this)
		...
		_trigger : function(p, q) {
			var o = q || this.date, n = new Date(o.getTime()
					+ (o.getTimezoneOffset() * 60000));
		...
		setDate : function(n) {
			this.setUTCDate(new Date(n.getTime()
					- (n.getTimezoneOffset() * 60000)))
		},
		...
		update : function() {
			...
			if (arguments
				...
				n = this.isInput ? this.element.val() : this.element
						.data("date")
						|| this.element.find("input").val();
				delete this.element.data().date
			}
		...
		fillDow : function() {
			...
			while (o < this.o.weekStart + 7) {
				p += '<th class="dow">' + b[this.o.language].daysMin[(o++) % 7]
						+ "</th>"
			}
			p += "</tr>";
		...
		fillMonths : function() {
			...
			while (n < 12) {
				o += '<span class="month">'
						+ b[this.o.language].monthsShort[n++] + "</span>"
			}
		...
		getClassNames : function(q) {
			var o = [], r = this.viewDate.getUTCFullYear(), s = this.viewDate
					.getUTCMonth(), n = this.date.valueOf(), p = new Date();
			if (q.getUTCFullYear() < r
					|| (q.getUTCFullYear() == r && q.getUTCMonth() < s)) {
				o.push("old")
			} else {
				if (q.getUTCFullYear() > r
						|| (q.getUTCFullYear() == r && q.getUTCMonth() > s)) {
			...
			if (this.o.todayHighlight && q.getUTCFullYear() == p.getFullYear()
					&& q.getUTCMonth() == p.getMonth()
					&& q.getUTCDate() == p.getDate()) {
				o.push("today")
			}
			if (n && q.valueOf() == n) {
				o.push("active")
			}
			if (q.valueOf() < this.o.startDate
					|| q.valueOf() > this.o.endDate
					|| g.inArray(q.getUTCDay(), this.o.daysOfWeekDisabled) !== -1) {
				o.push("disabled")
			}
			if (this.range) {
		...
		fill : function() {
			var F = new Date(this.viewDate), w = F.getUTCFullYear(), G = F
					.getUTCMonth(), A = this.o.startDate !== -Infinity
					? this.o.startDate.getUTCFullYear()
					: -Infinity, E = this.o.startDate !== -Infinity
					? this.o.startDate.getUTCMonth()
					: -Infinity, t = this.o.endDate !== Infinity
					? this.o.endDate.getUTCFullYear()
					: Infinity, B = this.o.endDate !== Infinity
					? this.o.endDate.getUTCMonth()
					: Infinity, u = this.date && this.date.valueOf(), q;
			this.picker.find(".datepicker-days thead th.datepicker-switch")
					.text(b[this.o.language].months[G] + " " + w);
			this.picker.find("tfoot th.today").text(b[this.o.language].today)
					.toggle(this.o.todayBtn !== false);
			this.picker.find("tfoot th.clear").text(b[this.o.language].clear)
					.toggle(this.o.clearBtn !== false);
			this.updateNavArrows();
			this.fillMonths();
			var I = k(w, G - 1, 28, 0, 0, 0, 0), D = i.getDaysInMonth(I
			...
			while (I.valueOf() < n) {
				if (I.getUTCDay() == this.o.weekStart) {
					...
					if (this.o.calendarWeeks) {
						var o = new Date(+I
								+ (this.o.weekStart - I.getUTCDay() - 7) % 7
								* 86400000), r = new Date(+o
								+ (7 + 4 - o.getUTCDay()) % 7 * 86400000), p = new Date(+(p = k(
								r.getUTCFullYear(), 0, 1))
								+ (7 + 4 - p.getUTCDay()) % 7 * 86400000), x = (r - p)
								/ 86400000 / 7 + 1;
			...
			this.picker.find(".datepicker-days tbody").empty().append(v
					.join(""));
			var J = this.date && this.date.getUTCFullYear();
			var s = this.picker.find(".datepicker-months").find("th:eq(1)")
					.text(w).end().find("span").removeClass("active");
			if (J && J == w) {
			...
			var H = this.picker.find(".datepicker-years").find("th:eq(1)")
					.text(w + "-" + (w + 9)).end().find("td");
			w -= 1;
			for (var C = -1; C < 11; C++) {
				v += '<span class="year'
						+ (C == -1 ? " old" : C == 10 ? " new" : "")
		...
		updateNavArrows : function() {
			...
			var p = new Date(this.viewDate), n = p.getUTCFullYear(), o = p
					.getUTCMonth();
			switch (this.viewMode) {
				case 0 :
					if (this.o.startDate !== -Infinity
							&& n <= this.o.startDate.getUTCFullYear()
							&& o <= this.o.startDate.getUTCMonth()) {
				...
				switch (s[0].nodeName.toLowerCase()) {
					case "th" :
						switch (s[0].className) {
							...
							case "prev" :
							case "next" :
								var n = i.modes[this.viewMode].navStep
										* (s[0].className == "prev" ? -1 : 1);
								switch (this.viewMode) {
									case 0 :
							...
							case "today" :
								var o = new Date();
								o = k(o.getFullYear(), o.getMonth(), o
												.getDate(), 0, 0, 0);
								this.showMode(-2);
								var p = this.o.todayBtn == "linked"
										? null
										: "view";
								this._setDate(o, p);
								break;
					...
					case "td" :
						if (s.is(".day") && !s.is(".disabled")) {
							...
							var u = this.viewDate.getUTCFullYear(), t = this.viewDate
									.getUTCMonth();
							if (s.is(".old")) {
								if (t === 0) {
									t = 11;
		...
		moveMonth : function(n, o) {
			...
			var r = new Date(n.valueOf()), v = r.getUTCDate(), s = r
					.getUTCMonth(), q = Math.abs(o), u, t;
			o = o > 0 ? 1 : -1;
			if (q == 1) {
				t = o == -1 ? function() {
		...
		showMode : function(n) {
			...
			this.picker.find(">div").hide().filter(".datepicker-"
					+ i.modes[this.viewMode].clsName).css("display", "block");
			this.updateNavArrows()
		}
	};
	var m = function(o, n) {
		...
		delete n.inputs;
		g(this.inputs).datepicker(n).bind("changeDate",
				g.proxy(this.dateUpdated, this));
	...
	m.prototype = {
		...
		dateUpdated : function(q) {
			var r = g(q.target).data("datepicker"), p = r.getUTCDate(), o = g
					.inArray(q.target, this.inputs), n = this.inputs.length;
			if (o == -1) {
				return
			}
			if (p < this.dates[o]) {
	...
	function f(q, t) {
		var s = g(q).data(), n = {}, r, p = new RegExp("^" + t.toLowerCase()
				+ "([A-Z])"), t = new RegExp("^" + t.toLowerCase());
		for (var o in s) {
			if (t.test(o)) {
	...
	var j = g.fn.datepicker = function(q) {
		...
		var p, n;
		this.each(function() {
			var y = g(this), w = y.data("datepicker"), s = typeof q == "object"
					&& q;
			if (!w) {
				var u = f(this, "date"), r = g.extend({}, e, u, s), t = a(r.language), v = g
						.extend({}, e, t, u, s);
	...
	var i = {
		...
		getDaysInMonth : function(n, o) {
			return [31, (i.isLeapYear(n) ? 29 : 28), 31, 30, 31, 30, 31, 31,
					30, 31, 30, 31][o]
		},
		validParts : /dd?|DD?|mm?|MM?|yy(?:yy)?/g,
		nonpunctuation : /[^ -\/:-@\[\u3400-\u9fff-`{-~\t\n\r]+/g,
		parseFormat : function(p) {
			var n = p.replace(this.validParts, "\0").split("\0"), o = p
		...
		parseDate : function(q, A, u) {
			...
			if (/^[\-+]\d+[dmwy]([\s,]+[\-+]\d+[dmwy])*$/.test(q)) {
				...
				return k(q.getUTCFullYear(), q.getUTCMonth(), q.getUTCDate(),
						0, 0, 0)
			}
			var t = q && q.match(this.nonpunctuation) || [], q = new Date(), y = {}, z = [
					"yyyy", "yy", "M", "MM", "m", "mm", "d", "dd"], B = {
				yyyy : function(E, s) {
						...
						switch (n) {
							case "MM" :
								w = g(b[u].months).filter(function() {
									var s = this.slice(0, t[v].length), E = t[v]
											.slice(0, s.length);
									return s == E
								});
	...
	i.template = '<div class="datepicker"><div class="datepicker-days"><table class=" table-condensed">'
			+ i.headTemplate
			+ "<tbody></tbody>"
			+ i.footTemplate
	...
	g(document).on("focus.datepicker.data-api click.datepicker.data-api",
			'[data-provide="datepicker"]', function(o) {
				var n = g(this);
...
(function(b) {
	...
	b.extend(a.prototype, {
				initPicker : function(c, d) {
					...
					c.datepicker = b.fn.editableutils.tryParseJson(
							c.datepicker, true);
					this.options.datepicker = b.extend({}, d.datepicker,
							c.datepicker, {
								format : this.options.viewformat
							});
					this.options.datepicker.language = this.options.datepicker.language
							|| "en";
					this.dpg = b.fn.bdatepicker.DPGlobal;
					this.parsedFormat = this.dpg
							.parseFormat(this.options.format);
					this.parsedViewFormat = this.dpg
							.parseFormat(this.options.viewformat)
				},
				render : function() {
					this.$input.bdatepicker(this.options.datepicker);
					if (this.options.clear) {
						this.$clear = b('<a href="#"></a>')
								.html(this.options.clear).click(b.proxy(
										function(c) {
											c.preventDefault();
											c.stopPropagation();
											this.clear()
						...
						this.$tpl.parent()
								.append(b('<div class="editable-clear">')
										.append(this.$clear))
					}
				},
				value2html : function(d, c) {
					var e = d ? this.dpg.formatDate(d, this.parsedViewFormat,
				...
				value2str : function(c) {
					return c ? this.dpg.formatDate(c, this.parsedFormat,
							this.options.datepicker.language) : ""
				},
				str2value : function(c) {
				...
				autosubmit : function() {
					this.$input.on("mouseup", ".day", function(d) {
								if (b(d.currentTarget).is(".old")
										|| b(d.currentTarget).is(".new")) {
									return
								}
				...
				parseDate : function(f, d) {
					var c = null, e;
					if (f) {
						c = this.dpg.parseDate(f, d,
								this.options.datepicker.language);
						if (typeof f === "string") {
							e = this.dpg.formatDate(c, d,
									this.options.datepicker.language);
							if (f !== e) {
								c = null
							}
...
(function(b) {
	...
	b.extend(a.prototype, {
				...
				value2input : function(c) {
					this.$input.val(c ? this.dpg.formatDate(c,
...
(function(a) {
	...
	a.extend(b.prototype, {
		initPicker : function(c, d) {
			...
			c.datetimepicker = a.fn.editableutils.tryParseJson(
					c.datetimepicker, true);
			this.options.datetimepicker = a.extend({}, d.datetimepicker,
					c.datetimepicker, {
						format : this.options.viewformat
					});
			this.options.datetimepicker.language = this.options.datetimepicker.language
					|| "en";
			this.dpg = a.fn.datetimepicker.DPGlobal;
			this.parsedFormat = this.dpg.parseFormat(this.options.format,
					this.options.formatType);
			this.parsedViewFormat = this.dpg.parseFormat(
					this.options.viewformat, this.options.formatType)
		},
		render : function() {
			...
			if (this.options.clear) {
				this.$clear = a('<a href="#"></a>').html(this.options.clear)
						.click(a.proxy(function(c) {
									c.preventDefault();
									c.stopPropagation();
									this.clear()
								}, this));
				this.$tpl.parent().append(a('<div class="editable-clear">')
						.append(this.$clear))
			}
		},
		value2html : function(d, c) {
			var e = d ? this.dpg.formatDate(this.toUTC(d),
					this.parsedViewFormat,
		...
		value2str : function(c) {
			return c ? this.dpg.formatDate(this.toUTC(c), this.parsedFormat,
					this.options.datetimepicker.language,
					this.options.formatType) : ""
		},
		...
		toUTC : function(c) {
			return c
					? new Date(c.valueOf() - c.getTimezoneOffset() * 60000)
					: c
		},
		fromUTC : function(c) {
			return c
...
(function(a) {
	...
	a.extend(b.prototype, {
				renderList : function() {
					this.$input = this.$tpl.is("input") ? this.$tpl : this.$tpl
				...
				value2htmlFinal : function(e, d) {
					if (this.getIsObjects()) {
						var c = a.fn.editableutils.itemsByValue(e,
								this.sourceData);
				...
				value2input : function(d) {
					if (this.getIsObjects()) {
						var c = a.fn.editableutils.itemsByValue(d,
								this.sourceData);
						this.$input.data("value", d).val(c.length
								? c[0].text
								: "")
					} else {
						this.$input.val(d)
				...
				input2value : function() {
					if (this.getIsObjects()) {
						var d = this.$input.data("value"), c = a.fn.editableutils
								.itemsByValue(d, this.sourceData);
						if (c.length
								&& c[0].text.toLowerCase() === this.$input
										.val().toLowerCase()) {
							return d
						} else {
							return null
				...
				matcher : function(c) {
					return a.fn.typeahead.Constructor.prototype.matcher.call(
							this, c.text)
				},
				sorter : function(e) {
					var f = [], d = [], c = [], g, h;
				...
				highlighter : function(c) {
					return a.fn.typeahead.Constructor.prototype.highlighter
							.call(this, c.text)
				},
				updater : function(c) {
