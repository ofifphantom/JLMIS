			...
			this.$file.ace_file_input(this.options.image).on("change",
					function() {
						var d = (c.$file.val() || c.$file
								.data("ace_input_files")) ? Math.random() + ""
...
(function(a) {
	...
	a.extend(b.prototype, {
				render : function() {
					...
					this.$tpl.parent().find(".wysiwyg-editor").show()
							.ace_wysiwyg({
							...
							.closest(".editable-container").css({
										display : "block"
									});
					if (this.options.wysiwyg && this.options.wysiwyg.css) {
						this.$tpl.closest(".editable-wysiwyg")
								.css(this.options.wysiwyg.css)
					}
				},
				...
				var d = b.extend(this.options.slider, {
					...
					slide : function(g, h) {
						...
						if (h.handle.firstChild == null) {
							b(h.handle)
									.append("<div class='tooltip top in' style='display:none;top:-38px;left:-5px;'><div class='tooltip-arrow'></div><div class='tooltip-inner'></div></div>")
						}
						b(h.handle.firstChild).show().children().eq(1).text(i)
					}
				});
				this.$input.parent().addClass("editable-slider").css("width",
						e + "px").slider(d)
			}
		},
		value2html : function(d, c) {
