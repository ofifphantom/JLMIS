...
<script src="${pageContext.request.contextPath}/junlin/js/photoload.js"
				...
				<div class="opration-div clearfix">
					<span class="r_f">
						<button type="button" class="btncss jl-btn-defult"
							onclick="flashSaleEduce()" style="margin-right: 0;">
							<i class="fa fa-download"></i> 导出
						</button>

					</span> <span class="r_f">
						<button type="button" class="btncss jl-btn-defult"
							onclick="flashSaleDel()">
							<i class="fa fa-trash"></i> 删除
						</button>
					</span> <span class="r_f">
						<button type="button" class="btncss jl-btn-importent"
							onclick="flashSaleAdd()">
	...
	<div id="editDiv" style="display: none;">
		<form class="container" id="inserOrUpdateDiv">
			<div class="form-group">
				...
				<label class="col-xs-2 control-label">分类：</label> <input type="text"
					class="hidden" id="type" name="type" value="3" /> <input
					type="text" class="hidden" id="urlType" name="urlType" value="1" />
				<div class="col-xs-6">限时抢购</div>
			</div>
			<div class="form-group">
				<div class="col-xs-2"></div>
				...
				<div class="col-xs-6">
					<input type="text" class="form-control" id="name" name="name"
						maxlength="100" placeholder="请输入限时抢购图名称" />
				</div>
			</div>
			<div class="form-group">
				<div class="col-xs-2"></div>
				<label for="" class="col-xs-2 control-label">秒杀活动名称：</label>
				<div id="activityDiv">
					<div class="col-xs-6">
						<select class="form-control" id="activityName" name="activityId">
							<option value="-1">--秒杀活动名称(编号)--</option>
				...
				<!--		<div  class="col-xs-6" id="goodsDiv">
					...
					<div class="col-xs-4">
						<select class="form-control" id="classificationTwo" onchange="loadGoodsName(this)" name="classificationTwo" style="border-left:0;border-right:0;">
			...
			<div class="form-group">
				<div class="col-xs-2"></div>
				<label for="" class="col-xs-2 control-label">有效期：</label>
				<div class="col-xs-6">
					<input type="text" class="form-control" id="effect_time"
						name="effectTime" placeholder="请选择有效期" readonly="" lay-key="1" />
				</div>
			</div>
			<div class="form-group">
				<div class="col-xs-4"></div>
				<div class="col-xs-6 tip">注：<br/>1.上传的图片大小应小于2M,图片尺寸建议为1500*390（单位：像素）<br/>2.该页面所有字段均为必填</div>
			</div>
			<div class="form-group">
				<div class="col-xs-12 text-center">
					<input type="text" class="hidden" id="flag" name="flag" />
					<button type="button" class="btncss jl-btn-importent class-add"
						id="saveAndEffect" onclick="addAd('saveAndEffect')">提交并生效</button>
					<button type="button" class="btncss jl-btn-defult" id="save"
						onclick="addAd('save')">保存</button>
				</div>
			</div>
...
<script>
	...
	jQuery(function($) {
		...
		//下拉列表赋值  活动名称
		 $.ajax({
			url :'<%=basePath%>backgroundConfiguration/activity/activityInformation/getAllPassActivityInformationByType2' ,
			type : "POST",
			dataType : "json",
			success : function(data) {
				for ( var i = 0; i < data.length; i++) {
					var option = $("<option value='"+data[i].id+"'>"+ data[i].name+"("+data[i].identifier +")</option>");
			...
			.dataTable({
				...
				"aoColumns": [{
						...
						"mRender": function(data, type, row) {
							...
							return '<td><input type="button" class="btncss edit" onclick=\'flashSaleEffect(this,'+ JSON.stringify(row) +')\' value="生效" /></td>';
						}
					}
		...
		function addAd(flag){
			...
			if($("#name").val() == "") {
				...
				laywarn("跳转类型不能为空!");
				return false;
			}else if($("#activityName").val() =="-1" ) {
				laywarn("请选择秒杀活动!");
				return false;
			}else if($("#effect_time").val() == "") {
				laywarn("请选择有效期!");
				return false;
			}
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data.success) {
						if(flag == "saveAndEffect"){
							laysuccess("信息保存成功并已生效!");
						}else{
							laysuccess(data.msg);
						}
		...
		/*生效*/
		function flashSaleEffect(thisbtn,data) {
