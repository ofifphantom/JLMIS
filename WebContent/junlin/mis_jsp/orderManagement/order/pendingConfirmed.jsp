...
<head>
	...
	jQuery(function($) {
		...
		function order(id) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(data.offlinePayment.state==4){
						$("#look_orderState").html("已通过审核,待确认");
					}
					else if(data.offlinePayment.state==6){
						$("#look_orderState").html("已通过确认");
					}
					else{
						$("#look_orderState").html("未通过确认");
					}
		...
		function orderPass(rowData){
			publicMessageLayer("该订单确认通过",function(){
				$.ajax({
		...
		function orderRefuse(rowData){
			publicMessageLayer("该订单确认不通过",function(){
				$.ajax({
					url :'<%=basePath%>orderManagement/order/order/auditingOrVerifyOrderById',
					type: "POST",
					dataType: "json",
