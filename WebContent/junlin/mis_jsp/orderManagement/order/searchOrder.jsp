...
<head>
		...
		<div id="editDiv" style="display: none;">
			<form class="container">
				...
				<div class="form-group" id="offline_type">
					...
					<div class="col-xs-4 imgbox" id="look_payMessage">
					</div>
				</div>
				<div class="form-group">
					<div class="col-xs-3"></div>
					...
					<div class="col-xs-7" id="look_orderStateDetail">
						<ul>
							<li>2017-04-29 已签收</li>
							<li>2017-04-29 已发货 车牌号：A4578992</li>
	...
	jQuery(function($) {
				...
				"ajax": {
					...
					"data": function(d) {
						return $.extend({}, d, {
							...
							"orderState": $("#search_orderState").val(),
							"page":1,
...

					{
						...
						"mRender": function(data, type, row) {
							...
							if(row.orderState==4){
								...
								+'<input type="button" class="btncss edit" onclick=\'orderDel(' + JSON.stringify(row) + ')\' value="确认取消" />'
								+'<input type="button" class="btncss edit" onclick=\'orderReturn(' + JSON.stringify(row) + ')\' value="恢复" /></td>'
							}else if(row.orderState==1){
								return '<td><input type="button" class="btncss edit" onclick=\'contactModify(' + JSON.stringify(row) + ')\' value="修改收货信息" />'
								+'<input type="button" class="btncss edit" onclick="order('+data+')" value="详情" /></td>'
							}else{
		...
		function order(id) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					case 0:
						$("#look_orderState").html("待支付");
						break;
					case 1:
						$("#look_orderState").html("待发货");
						break;
					case 2:
						$("#look_orderState").html("待收货");
						break;
					case 3:
						$("#look_orderState").html("已完成");
						break;
					case 4:
						$("#look_orderState").html("已取消");
						break;
					case 5:
		...
		function orderDel(data){
			//确认取消订单时，订单状态将变为5(已关闭)
			publicMessageLayer("取消该订单",function(){
				$.ajax({
					url: '<%=basePath%>orderManagement/order/order/updateOrderStatusByOrderId',
					type: "POST",
					dataType: "json",
					async: false,
					cache: false,
		...
		function orderReturn(data){
			publicMessageLayer("恢复该订单至待支付状态",function(){
				$.ajax({
					url: '<%=basePath%>orderManagement/order/order/updateOrderStatusByOrderId',
					type: "POST",
					dataType: "json",
		...
		function contactModify(data){
			$("#contact_consignee").val(data.consigneeName);
			$("#contact_information").val(data.consigneeTel);
			$("#contact_address").val(data.consigneeAddress);
			layer.open({
				type: 1,
				title: "修改",
				closeBtn: 1,
				...
				yes:function(index){
					if(checkContactModifyDiv()){
						$.ajax({
							url: '<%=basePath%>orderManagement/order/order/updateConsigneeMsgByOrderId',
							type: "POST",
							dataType: "json",
							async: false,
							cache: false,
							data: {
								"id": data.id,
								"consigneeName":$("#contact_consignee").val(),
								"consigneeTel":$("#contact_information").val(),
								"consigneeAddress":$("#contact_address").val(),
								"orderNo":data.orderNo 
							},
							...
							success: function(data) {
								/* 进销存销售订单修改 */
								$.ajax({
									url: '<%= PSIPath%>/JLPSI/sales/salesOrder/updateReceiverMsg',
									type: "POST",
									dataType: "json",
									async: false,
									cache: false,
									data: {
										"missOrderId": data.id,
										"consignee":$("#contact_consignee").val(),
										"phone":$("#contact_information").val(),
										"receiptGoodsPlace":$("#contact_address").val()
